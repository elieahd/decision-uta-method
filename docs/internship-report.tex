...
\subsection{Experimentation about the variant of UTA}
...
Although I couldn't find studies about how to generate realistic random alternatives I was able to find studies about the UTA method. Those studies were stated. Another aspect of the research was the simulation part of the uta-calculator. The simulation study will be discussed in the next section.\\
...
We generated a value function called $v^R$, then we ordered a list of 50 alternatives by the value function generated.\\
...
Then we took \textit{alternativesToCompare} and ordered them according to the two value functions $v^R$ and $v^T$ and we compared the difference of ranking between the two list.\\
We repeated the upper process 100 times and then we calculated the mean and the standard deviation. \\
This following simulation was repeated by varying the number of criteria between $1,2,3,4,5$ and by varying the number of alternatives we wanted to take as example $10, 20, 30, 40$.\\
...
The more we added criteria the more the differences grew between the two lists. Each time we added 1 criterion the difference of ranking has augmented between $20-30 \%$\\

...
\chapter{Example}
The implementation of UTASTAR algorithm is illustrated by the example from the Introduction chapter : \textbf{buying a new car}.\\ The DM is interested only int eh following criteria:
\begin{itemize}
\item price (in Euro)
\item comfort ($0$, +, ++, +++) \textit{0 being not comfortable and +++ very comfortable}
\item safety ($1, 2, 3, 4, 5$) \textit{1 being not safety and 5 safe}
...
\begin{center}
 ...
 \hline
 Citroen C4 (c4) & 15\,000& ++ & 2 & 1\\ 
 \hline
 Peugeot 208 GT (p208) & 25\,000 & + & 3 & 2\\
 \hline
 Peugeot 308 berline (p308)& 18\,500 & 0 & 3 & 3\\
 \hline
 Nissan Sentra (ns) & 17\,000 & +++ & 4 & 4 \\ 
 \hline
\end{tabular}
\end{center}
...
\item $\Delta (p308, ns) = - 0.1w_{12} - w_{21} - w_{22} - w_{23} - 0.5w_{32} -\sigma _{p308}^{+} +\sigma _{p308}^{-} +\sigma _{ns}^{+} - \sigma _{ns}^{-} $
\end{itemize}
...
\begin{equation}
	...
	\end{cases}
\end{equation}
An optimal solution is $w_{12} = 0.318$, $w_{21} = 0.146$, $w_{31} = 0.536$ with $\sum_{a \in A} \sigma _{a}^{+} + \sigma _{a}^{-} = 0.227$ with $\sigma _{ns}^{+} = 0.227$.\\
